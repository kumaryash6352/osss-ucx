...
void shmemc_ctx_put_nbi(shmem_ctx_t ctx, void *dest, const void *src,
                        size_t nbytes, int pe);
void shmemc_ctx_get_nbi(shmem_ctx_t ctx, void *dest, const void *src,
                        size_t nbytes, int pe);

void shmemc_ctx_put_signal(shmem_ctx_t ctx, void *dest, const void *src,
                           size_t nbytes, uint64_t *sig_addr, uint64_t signal,
                           int sig_op, int pe);

void shmemc_ctx_put_signal_nbi(shmem_ctx_t ctx, void *dest, const void *src,
...
void shmemc_ctx_swap(shmem_ctx_t ctx, void *target, void *value, size_t vals,
                     int pe, void *retp);

void shmemc_ctx_cswap(shmem_ctx_t ctx, void *target, void *cond, void *value,
                      size_t vals, int pe, void *retp);

/*
 * adds and incs (== add1)
 */

void shmemc_ctx_add(shmem_ctx_t ctx, void *target, void *value, size_t vals,
                    int pe);

void shmemc_ctx_fadd(shmem_ctx_t ctx, void *target, void *value, size_t vals,
                     int pe, void *retp);
void shmemc_ctx_fadd_nbi(shmem_ctx_t ctx, void *target, void *value,
                         size_t vals, int pe, void *retp);
...
#define SHMEMC_CTX_DECL_BITWISE(_op)                                           \
  void shmemc_ctx_##_op(shmem_ctx_t ctx, void *target, void *value,            \
                        size_t vals, int pe);
...
#define SHMEMC_CTX_DECL_FETCH_BITWISE(_op)                                     \
  void shmemc_ctx_fetch_##_op(shmem_ctx_t ctx, void *target, void *value,      \
                              size_t vals, int pe, void *retp);
...
#define SHMEMC_CTX_DECL_FETCH_BITWISE_NBI(_op)                                 \
  void shmemc_ctx_fetch_##_op##_nbi(shmem_ctx_t ctx, void *target,             \
                                    void *value, size_t vals, int pe);
 ...
 * set/fetch
 */
void shmemc_ctx_set(shmem_ctx_t ctx, void *tp, size_t ts, void *vp, size_t vs,
                    int pe);
void shmemc_ctx_fetch(shmem_ctx_t ctx, void *tp, size_t ts, int pe, void *valp);
void shmemc_ctx_fetch_nbi(shmem_ctx_t ctx, void *tp, size_t ts, int pe,
                          void *valp);
...
#define SHMEMC_CTX_TEST(_size, _opname)                                        \
  int shmemc_ctx_test_##_opname##_size(shmem_ctx_t ctx, int##_size##_t *var,   \
                                       int##_size##_t value);
...
#define SHMEMC_CTX_WAIT_UNTIL(_size, _opname)                                  \
  void shmemc_ctx_wait_until_##_opname##_size(                                 \
      shmem_ctx_t ctx, int##_size##_t *var, int##_size##_t value);
...
#define SHMEMC_CTX_TEST_ALL(_size, _opname)                                    \
  int shmemc_ctx_test_all_##_opname##_size(                                    \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, const int *status, \
      int##_size##_t value);
...
#define SHMEMC_CTX_TEST_SOME(_size, _opname)                                   \
  size_t shmemc_ctx_test_some_##_opname##_size(                                \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, size_t *idxs,      \
      const int *status, int##_size##_t value);
...
#define SHMEMC_CTX_TEST_ANY(_size, _opname)                                    \
  size_t shmemc_ctx_test_any_##_opname##_size(                                 \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, const int *status, \
      int##_size##_t value);
...
#define SHMEMC_CTX_WAIT_UNTIL_ALL(_size, _opname)                              \
  void shmemc_ctx_wait_until_all_##_opname##_size(                             \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, const int *status, \
      int##_size##_t value);
...
#define SHMEMC_CTX_WAIT_UNTIL_SOME(_size, _opname)                             \
  size_t shmemc_ctx_wait_until_some_##_opname##_size(                          \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, size_t *idxs,      \
      const int *status, int##_size##_t value);
...
#define SHMEMC_CTX_WAIT_UNTIL_ANY(_size, _opname)                              \
  size_t shmemc_ctx_wait_until_any_##_opname##_size(                           \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, const int *status, \
      int##_size##_t value);
...
#define SHMEMC_CTX_TEST_ALL_VECTOR(_size, _opname)                             \
  int shmemc_ctx_test_all_vector_##_opname##_size(                             \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, const int *status, \
      void *values);
...
#define SHMEMC_CTX_TEST_SOME_VECTOR(_size, _opname)                            \
  size_t shmemc_ctx_test_some_vector_##_opname##_size(                         \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, size_t *idxs,      \
      const int *status, void *values);
...
#define SHMEMC_CTX_TEST_ANY_VECTOR(_size, _opname)                             \
  size_t shmemc_ctx_test_any_vector_##_opname##_size(                          \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, const int *status, \
      void *values);
...
#define SHMEMC_CTX_WAIT_UNTIL_ALL_VECTOR(_size, _opname)                       \
  void shmemc_ctx_wait_until_all_vector_##_opname##_size(                      \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, const int *status, \
      void *values);
...
#define SHMEMC_CTX_WAIT_UNTIL_SOME_VECTOR(_size, _opname)                      \
  size_t shmemc_ctx_wait_until_some_vector_##_opname##_size(                   \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, size_t *idxs,      \
      const int *status, void *values);
...
#define SHMEMC_CTX_WAIT_UNTIL_ANY_VECTOR(_size, _opname)                       \
  size_t shmemc_ctx_wait_until_any_vector_##_opname##_size(                    \
      shmem_ctx_t ctx, int##_size##_t *vars, size_t nelems, const int *status, \
      void *values);
...
#define SHMEMC_DECL_FCOLLECT_SIZE(_size)                                       \
  void shmemc_fcollect##_size(void *target, const void *source, size_t nelems, \
...
#define SHMEMC_DECL_COLLECT_SIZE(_size)                                        \
  void shmemc_collect##_size(void *target, const void *source, size_t nelems,  \
                             int PE_start, int logPE_stride, int PE_size,      \
                             long *pSync);
...
inline static int shmemc_my_pe(void) {
  return shmemc_team_my_pe(SHMEM_TEAM_WORLD);
}

inline static int shmemc_n_pes() { return shmemc_team_n_pes(SHMEM_TEAM_WORLD); }
...
#define shmemc_swap(...) shmemc_ctx_swap(SHMEM_CTX_DEFAULT, __VA_ARGS__)

#define shmemc_cswap(...) shmemc_ctx_cswap(SHMEM_CTX_DEFAULT, __VA_ARGS__)

#define shmemc_add(...) shmemc_ctx_add(SHMEM_CTX_DEFAULT, __VA_ARGS__)
#define shmemc_inc(...) shmemc_ctx_inc(SHMEM_CTX_DEFAULT, __VA_ARGS__)

#define shmemc_fadd(...) shmemc_ctx_fadd(SHMEM_CTX_DEFAULT, __VA_ARGS__)
#define shmemc_fadd_nbi(...) shmemc_ctx_fadd_nbi(SHMEM_CTX_DEFAULT, __VA_ARGS__)
#define shmemc_finc(...) shmemc_ctx_finc(SHMEM_CTX_DEFAULT, __VA_ARGS__)
#define shmemc_finc_nbi(...) shmemc_ctx_finc_nbi(SHMEM_CTX_DEFAULT, __VA_ARGS__)

#define shmemc_and(...) shmemc_ctx_and(SHMEM_CTX_DEFAULT, __VA_ARGS__)
#define shmemc_or(...) shmemc_ctx_or(SHMEM_CTX_DEFAULT, __VA_ARGS__)
#define shmemc_xor(...) shmemc_ctx_xor(SHMEM_CTX_DEFAULT, __VA_ARGS__)
...
#define shmemc_fetch_or(...) shmemc_ctx_fetch_or(SHMEM_CTX_DEFAULT, __VA_ARGS__)
